...
if (mysql_num_rows($result) != 0) {
    ...
    if (isset($_GET['sub_m1']) && $row['status'] == 0) {
		$amount = 6500;
		$user_id = $row['login'];
				...
				if (($pay_sum - $amount) >= 0) {
					...
					while ($sponsor[$i]!="" && $i<$n && !$flag_m) {
						...
						if (mysql_num_rows($result_x)<2) {	
							$res_xyz = mysql_query("select user_id from m0 where user_login='".$row['login']."'");
							if (mysql_num_rows($res_xyz) == 0) {
							mysql_query("insert into m0 (user_id, user_login, sponsor_login, type, post_time, pay) values (".$row['id'].", '".$row['login']."', '".$sponsor[$i]."', ".(mysql_num_rows($result_x)+1).", '".date('Y-m-d H:i:s')."', '1')");
							...
							if($_GET["pay_type"]=="balans")	{
								mysql_query("update users set akwa=akwa-6500, status=1, isStatusPaid=1 where login='".$row['login']."'");
							} else {
								mysql_query("update users set balans_turbo=balans_turbo-6500, status=1, isStatusPaid=1 where login='".$row['login']."'");
							}
							mysql_query("insert into transfer (login, amount, user_login, product, sent_time) values ('".$sponsor[$i]."', 000, '".$row['login']."', 'Стартовый', '".date('Y-m-d H:i:s')."')");
							$flag_m = true;
							$res_m = mysql_query("select * from m0 where user_login='".$sponsor[$i]."'");
							if (mysql_num_rows($res_m)>0) {
								...
								if($check_block['block_bonus'] == 0) {
									mysql_query("update users set akwa=akwa+500 where login='".$row_m['sponsor_login']."'");
									mysql_query("insert into transfer (login, amount, user_login, product, sent_time) values ('".$row_m['sponsor_login']."', 0, '".$row['login']."', 'Стартовый', '".date('Y-m-d H:i:s')."')");
									
								}
								check_binary_0($row_m['user_login']);
							}							
							echo mysql_error();
							$flag_k = false;
							$lot_no = 0;
        ...
        <div class="content mt-3">
            <div class="card">
                ...
                <div class="row">
					...
					if ($row['status'] == 0) {
					?>
					<a href="#" data-bs-toggle="modal" data-bs-target="#buyTypeMarketing" class="btn btn-light"> <i class="menu-icon fa fa-sitemap " style="font-size:18px;"></i>Вход 6500 тг</a><br><br>
					<?php
					}
