...
if (mysql_num_rows($result) <= 4) {
				...
				$pay_sum = $_GET["pay_type"]=="balans"? $row['akwa']: $row["balans_turbo"];
					...
					while ($sponsor[$i]!="" && $i<$n && !$flag_m) {
						...
						if (mysql_num_rows($result_x)<2) {	
							$res_xyz = mysql_query("select user_id from m4 where user_login='".$row['login']."'");
							if (mysql_num_rows($res_xyz) == 0) {
							mysql_query("insert into m4 (user_id, user_login, sponsor_login, type, post_time, pay) values (".$row['id'].", '".$row['login']."', '".$sponsor[$i]."', ".(mysql_num_rows($result_x)+1).", '".date('Y-m-d H:i:s')."', '1')");
	...
	mysql_query("update users set akwa=akwa-330000, status=5, isStatusPaid=1 where login='".$row['login']."'");
} else {
	mysql_query("update users set balans_turbo=balans_turbo-330000, status=5, isStatusPaid=1 where login='".$row['login']."'");
}
							mysql_query("insert into transfer (login, amount, user_login, product, sent_time) values ('".$sponsor[$i]."', 00000, '".$row['login']."', 'BRONZE Level 4', '".date('Y-m-d H:i:s')."')");
							$flag_m = true;
							...
							if (mysql_num_rows($res_m)>0) {
								...
								if($check_block['block_bonus'] == 0) {
									...
									mysql_query("insert into transfer (login, amount, user_login, product, sent_time) values ('".$row_m['sponsor_login']."', 8000, '".$row['login']."', 'BRONZE Level 4', '".date('Y-m-d H:i:s')."')");
        ...
        <div class="content mt-3">
           <div class="card">
                ...
                <div class="row">
					...
					if ($row['status'] <= 4) {
					?>
					<a href="#" data-bs-toggle="modal" data-bs-target="#buyTypeMarketing" class="btn btn-success"> <i class="menu-icon fa fa-sitemap " style="font-size:18px;"></i>Вход 330000 тг</a><br><br>
					...
					<span class="badge badge-success">BRONZE Level 4</span>
