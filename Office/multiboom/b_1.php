...
if (mysql_num_rows($result) != 0) {
    ...
    if (isset($_GET['sub_qq1']) && $row['status1'] == 0) {
		$amount = 88000;
		$user_id = $row['login'];
		...
		//	$r++;
			while ($flag == false) {
				...
				//$res_qq = mysql_query("select * from m2 where sponsor_login='".$sponsor_login."' and type=".$k);
				...
				$pay_sum = $_GET["pay_type"]=="balans"? $row['akwa']: $row["balans_turbo"];
					...
					while ($sponsor[$i]!="" && $i<$n && !$flag_qq) {
						...
						if (mysql_num_rows($result_x)<2) {	
							$res_xyz = mysql_query("select user_id from qq1 where user_login='".$row['login']."'");
							if (mysql_num_rows($res_xyz) == 0) {
							mysql_query("insert into qq1 (user_id, user_login, sponsor_login, type, post_time, pay) values (".$row['id'].", '".$row['login']."', '".$sponsor[$i]."', ".(mysql_num_rows($result_x)+1).", '".date('Y-m-d H:i:s')."', '1')");
							...
							mysql_query("update users set akwa=akwa+000 where login='".$sponsor[$i]."'");	
							if($_GET["pay_type"]=="balans")	{
	mysql_query("update users set akwa=akwa-88000, status1=1, isStatusPaid1=1 where login='".$row['login']."'");
} else {
	mysql_query("update users set balans_turbo=balans_turbo-88000, status1=1, isStatusPaid1=1 where login='".$row['login']."'");
}
							mysql_query("insert into transfer (login, amount, user_login, product, sent_time) values ('".$sponsor[$i]."', 000, '".$row['login']."', 'MB Level 1', '".date('Y-m-d H:i:s')."')");
							$flag_qq = true;
							$res_qq = mysql_query("select * from qq1 where user_login='".$sponsor[$i]."'");
							if (mysql_num_rows($res_qq)>0) {
								...
								if($check_block['block_bonus'] == 0) {
									...
									mysql_query("insert into transfer (login, amount, user_login, product, sent_time) values ('".$row_qq['sponsor_login']."', 0, '".$row['login']."', 'MB Level 1', '".date('Y-m-d H:i:s')."')");
									
								}
								check_binary_1($row_qq['sponsor_login']);
							}							
							echo mysql_error();
							$flag_k = false;
							$lot_no = 0;
							...
							mysql_query("insert into konkurs ( user_id, lot_no, status1, post_time, upd_time ) values ( ".$row['id'].", ".$lot_no.", 0, '".date('Y-m-d H:i:s')."', '0000-00-00 00:00:00' )");
							}
						}
						else {
        ...
        <div class="content mt-3" style="background: #fff">
            <div class="animated fadeIn">
				...
				<div class="col-lg-12" style="text-align: center">
					...
					if ($row['status1'] == 0) {
					?>
					<a href="#" data-toggle="modal" data-target="#buyTypeMulti" class="btn btn-success"> <i class="menu-icon fa fa-sitemap " style="font-size:18px;"></i>Вход 88000 тг</a><br><br>
					...
					<div class="chart01" id="collapsable-example1" style="color: #fff;"></div>
