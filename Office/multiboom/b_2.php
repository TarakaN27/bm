...
if (mysql_num_rows($result) <= 1) {
    ...
    if (isset($_GET['sub_qq1'])) {
		...
		//	$r++;
			while ($flag == false) {
				...
				//$res_m = mysql_query("select * from qq2 where sponsor_login='".$sponsor_login."' and type=".$k);
				...
				$pay_sum = $_GET["pay_type"]=="balans"? $row['akwa']: $row["balans_turbo"];
					...
					while ($sponsor[$i]!="" && $i<$n && !$flag_qq) {
						...
						if (mysql_num_rows($result_x)<2) {	
							$res_xyz = mysql_query("select user_id from qq2 where user_login='".$row['login']."'");
							if (mysql_num_rows($res_xyz) == 0) {
							mysql_query("insert into qq2 (user_id, user_login, sponsor_login, type, post_time, pay) values (".$row['id'].", '".$row['login']."', '".$sponsor[$i]."', ".(mysql_num_rows($result_x)+1).", '".date('Y-m-d H:i:s')."', '1')");
	...
	mysql_query("update users set akwa=akwa-352000, status1=2, isStatusPaid1=1 where login='".$row['login']."'");
} else {
	mysql_query("update users set balans_turbo=balans_turbo-352000, status1=2, isStatusPaid1=1 where login='".$row['login']."'");
}
							mysql_query("insert into transfer (login, amount, user_login, product, sent_time) values ('".$sponsor[$i]."', 0, '".$row['login']."', 'MB Level 2', '".date('Y-m-d H:i:s')."')");
							$flag_qq = true;
							...
							if (mysql_num_rows($res_qq)>0) {
								...
								if($check_block['block_bonus'] == 0) {
									...
									mysql_query("insert into transfer (login, amount, user_login, product, sent_time) values ('".$row_qq['sponsor_login']."', 10000, '".$row['login']."', 'MB Level 2', '".date('Y-m-d H:i:s')."')");
        ...
        <div class="content mt-3" style="background: #fff">
            <div class="animated fadeIn">
				...
				<div class="col-lg-12" style="text-align: center">
					...
					if ($row['status1'] <= 1) {
					?>
					<a href="#" data-toggle="modal" data-target="#buyTypeMulti" class="btn btn-success"> <i class="menu-icon fa fa-sitemap " style="font-size:18px;"></i>Вход 352000 тг</a><br><br>
					...
					<?php
										...
										?>
									<div class="col-lg-12" style="text-align: center">
					<form method="post" action="b_2.php">
					...
					<span class="badge badge-white">Level 2</span>
