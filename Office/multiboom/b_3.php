...
if (mysql_num_rows($result) <= 3) {
	...
	if (isset($_GET['sub_qq1']) && $row['status1'] <= 3) {
		$amount = 1155000;
		$user_id = $row['login'];
				...
				$pay_sum = $_GET["pay_type"]=="balans"? $row['akwa']: $row["balans_turbo"];
					...
					while ($sponsor[$i]!="" && $i<$n && !$flag_m) {
						...
						if (mysql_num_rows($result_x)<2) {	
							$res_xyz = mysql_query("select user_id from qq3 where user_login='".$row['login']."'");
							if (mysql_num_rows($res_xyz) == 0) {
							mysql_query("insert into qq3 (user_id, user_login, sponsor_login, type, post_time, pay) values (".$row['id'].", '".$row['login']."', '".$sponsor[$i]."', ".(mysql_num_rows($result_x)+1).", '".date('Y-m-d H:i:s')."', '1')");
	...
	mysql_query("update users set akwa=akwa-1155000, status1=3, isStatusPaid1=1 where login='".$row['login']."'");
} else {
	mysql_query("update users set balans_turbo=balans_turbo-1155000, status1=3, isStatusPaid1=1 where login='".$row['login']."'");
}
							mysql_query("insert into transfer (login, amount, user_login, product, sent_time) values ('".$sponsor[$i]."', 0, '".$row['login']."', 'MB Level 3', '".date('Y-m-d H:i:s')."')");
							$flag_m = true;
							$res_m = mysql_query("select * from qq3 where user_login='".$sponsor[$i]."'");
							if (mysql_num_rows($res_m)>0) {
								...
								if($check_block['block_bonus'] == 0) {
									mysql_query("update users set akwa=akwa+50000 where login='".$row_m['sponsor_login']."'");
									mysql_query("insert into transfer (login, amount, user_login, product, sent_time) values ('".$row_m['sponsor_login']."', 50000, '".$row['login']."', 'MB Level 3', '".date('Y-m-d H:i:s')."')");
        ...
        <div class="content mt-3" style="background: #fff">
            <div class="animated fadeIn">
				...
				<div class="col-lg-12" style="text-align: center">
					...
					if ($row['status1'] <= 2) {
					?>
					<a href="#" data-toggle="modal" data-target="#buyTypeMulti" class="btn btn-success"> <i class="menu-icon fa fa-sitemap " style="font-size:18px;"></i>Вход 1155000 тг</a><br><br>
					...
					<div class="col-lg-12" style="text-align: center">
					<span class="badge badge-white">Level 3</span>
