				...
				$pay_sum = $_GET["pay_type"]=="balans"? $row['akwa']: $row["balans_turbo"];
					...
					while ($sponsor[$i]!="" && $i<$n && !$flag_m) {
						...
						if (mysql_num_rows($result_x)<2) {	
							$res_xyz = mysql_query("select user_id from qq5 where user_login='".$row['login']."'");
							if (mysql_num_rows($res_xyz) == 0) {
							mysql_query("insert into qq5 (user_id, user_login, sponsor_login, type, post_time, pay) values (".$row['id'].", '".$row['login']."', '".$sponsor[$i]."', ".(mysql_num_rows($result_x)+1).", '".date('Y-m-d H:i:s')."', '1')");
							mysql_query("update users set akwa=akwa+00000 where login='".$sponsor[$i]."'");
							if($_GET["pay_type"]=="balans")	{
	mysql_query("update users set akwa=akwa-0, status1=5, isStatusPaid1=1 where login='".$row['login']."'");
} else {
	mysql_query("update users set balans_turbo=balans_turbo-0, status1=5, isStatusPaid1=1 where login='".$row['login']."'");
}
							mysql_query("insert into transfer (login, amount, user_login, product, sent_time) values ('".$sponsor[$i]."', 00000, '".$row['login']."', 'MB Level 5', '".date('Y-m-d H:i:s')."')");
							$flag_m = true;
							$res_m = mysql_query("select * from qq5 where user_login='".$sponsor[$i]."'");
							if (mysql_num_rows($res_m)>0) {
								...
								if($check_block['block_bonus'] == 0) {
									mysql_query("update users set akwa=akwa+200000 where login='".$row_m['sponsor_login']."'");
									mysql_query("insert into transfer (login, amount, user_login, product, sent_time) values ('".$row_m['sponsor_login']."', 200000, '".$row['login']."', 'MB Level 5', '".date('Y-m-d H:i:s')."')");
        ...
        <div class="content mt-3">
           <div class="card">
				...
				<div class="col-lg-12" style="text-align: center">
					...
					<span class="badge badge-success mt-2">Level 5</span>
